...
with col2:
    ...
    st.subheader("Why three different plots?")
    st.markdown(
        """
    ...
    st.subheader("Which polls are used?")
    st.markdown(
        """
...
st.markdown(
    """
    There are many lessons to draw from these graphs, but the main and, apparently, atemporal ones are:
    
    1. **French people don't seem to like their presidents that much**: their median approval very rarely goes 
    ...
    2. However, **they can _really_ dislike their presidents**, François Hollande being the most prominent
    ...
    around the average, with only a small amount of variation. And Jacques Chirac (in his second term)
    is the only one who was more popular than average during the majority of his term.
     
    3. **Presidential approval's ceiling seems to slowly go down since the switch from a 7-year to a
    ...
    appearance of 24/7 news media and social networks (and the ensuing shortening of the news cycle).
    ...
    4. **It is usual for a president to experience a dip in popularity after 1-2 years in office**.
    It's just the honeymoon effect fading away, combined to 
    [reversion to the mean](https://en.wikipedia.org/wiki/Regression_toward_the_mean). Then they 
    oscillate around this mean, which is itself a combination of the historical baseline approval
    and a mean specific to the given president. Hollande's baseline for instance seemed to be much
    lower than Macron's and Sarkozy's, with those two being basically at the historical baseline.
    All that suggests that reacting to the most recent poll usually amounts to reacting to noise.
